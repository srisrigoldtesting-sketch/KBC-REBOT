...
def build_clients(settings: Settings):
    ...
    return (Client("kbc_bot", bot_token=settings.bot_token, **common),
            Client("kbc_premium", session_string=settings.string_session, no_updates=True, **common))
...
async def verify_telegram(bot, premium, settings: Settings):
    from pyrogram.enums import ChatMemberStatus, ChatType
    if not bot.me.is_bot:
        raise SetupError("BOT_TOKEN must authenticate a Telegram bot.")
    if premium.me.is_bot or not premium.me.is_premium:
        raise SetupError("STRING_SESSION must belong to an account with active Telegram Premium for 4GB uploads.")
    found = False
    async for dialog in premium.get_dialogs():
        if dialog.chat.id == settings.staging_chat_id:
            found = True
            break
    if not found:
        raise SetupError("The Premium account must join the private STAGING_CHAT_ID channel first.")
    for label, client in (("Bot", bot), ("Premium account", premium)):
